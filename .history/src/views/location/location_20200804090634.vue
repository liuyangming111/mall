  ...
  <div>
    ...
    <div class="background">
      ...
      <van-popup
        ...
        :style="{ height: '85%' }"
      >
        <div v-for="(item1,index) in search" :key="index" class="center"  @click="clickbtn(item1)">{{item1.name}}</div>
      </van-popup>
      ...
      <div class="hotCities">
        <div v-for="(item1, index) in hotCities" :key="index"  @click="clickbtn(item1)">{{item1.name}}</div>
      </div>
      <van-index-bar>
        <div v-for="(item,index) in cities" :key="index">
          <van-index-anchor :index="index" />
          <div v-for="(item1,index1) in item" :key="index1">
            <van-cell :title="item1.name" @click="clickbtn(item1)"/>
          </div>
        </div>
      </van-index-bar>
    </div>
...
export default {
  ...
  methods: {
    //每个路由zu'jin'a匿名为returns的都是返回为上一页
    returns() {
      this.$router.go(-1);
    },
    search1() {
    ...
    clickbtn(item1){
       sessionStorage.setItem("search",JSON.stringify(item1.name))
       this.$router.push("/")
    }
  },
  mounted() {
