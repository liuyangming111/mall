  ...
  <div>
    ...
    <div v-else>
      ...
      <div v-for="(item,index) in shopList" :key="index" class="commodity">
        ...
        <div class="border">
          <img :src="item.image_path" alt width="100px"  @click="$toDetails(item.cid)"/>
        </div>
        <div class="Prices">
          <div>{{item.name}}</div>
          <div>
      ...
      this.$api
        ...
        this.$api
          ...
          .then((res) => {
            ...
            this.$store.commit("setName", res.shopList.length)//购物车下标
