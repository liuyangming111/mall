<template>
  <div>
    ...
    <div class="information">
      ...
      <div v-if="nickname.length>0" class="sign_out">
        <div>欢迎您：{{nickname.userInfo.nickname}}</div>  <br>
        <div>退出登录</div>
      </div> 
      <div v-else>
...
export default {
  ...
  data() {
    return {
      nickname:{}
      user:JSON.parse(localStorage(u))
    };
  },
  methods: {
    clickbtn() {
  ...
  mounted() {
    this.nickname=JSON.parse(localStorage.getItem('user'))
