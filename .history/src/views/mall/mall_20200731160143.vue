<template>
  <div style="background-color: rgb(238,238,238)">
    <div class="top">
      <div class="city" @click="citys">{{city}}▼</div>
      <div>
      <van-search v-model="value" show-action placeholder="请输入搜索关键词" @change="Search" class="Searchs">
          <template #action>
            <div>搜索</div>
          </template>
        </van-search>
    ...
    <br><br><br>
    <div v-for="(item,index) in arr" :key="index" class="search"  @click="$toDetails(item.id)" v-html="item.name">{{item.name}}</div>
...
export default {
  ...
  methods: {
    ...
    Search(){
      this.$api.postkey_word(this. ).then(res=>{
        if(res.code===200){
