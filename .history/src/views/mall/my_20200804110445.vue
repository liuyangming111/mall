  ...
  <div>
    ...
    <div class="information">
      ...
      <div v-else class="sign_out">
        ...
        <div class="set_up" @click="personal"><van-icon name="setting" size="20px"/></div>
      </div>
    </div>
    ...
    <div v-else>
      <van-grid :column-num="5">
        ...
        <van-grid-item icon="logistics" text="待发货" />
        <van-grid-item icon="points" text="待收货" />
        <van-grid-item icon="good-job-o" text="评价" @click="evaluate" />
        <van-grid-item icon="like-o" text="已完成" @click="order"/>
      </van-grid>
    </div>
...
export default {
  ...
  data() {
    return {
      userInfo: {},
      user:JSON.parse(localStorage.getItem('user'))//我
    };
  },
  methods: {
    clickbtn() {
