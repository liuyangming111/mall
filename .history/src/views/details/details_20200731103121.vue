  ...
  mounted() {
    ...
    this.$api
      ...
      .then((res) => {
        ...
        let browse = JSON.parse(localStorage.getItem("browse"));
        if (browse === null) {
          let i = [];
          i.push(this.goodsOne);
          localStorage.setItem("browse", JSON.stringify(i));
        } else {
          let brr = [];
          if (
            browse.spme((item) => {
            ...
            arr.unshift(this.goodsOne);
            localStorage.setItem("browse",JSON)
          }
        }
