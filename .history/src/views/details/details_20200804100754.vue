<template>
  <div>
    ...
    <van-tabs v-model="active">
      ...
      <van-tab title="商品评论">
        <div v-if='comment.length===0' class="No_comments">暂无评论哦~~</div>
        <div v-for="(item, index) in comment" :key="index" v-else>
          <div class="time">
            <van-icon name="friends" color="red" size="20px" class="friends" />
            <div>
              <div>
                <div v-if="item.anonymous===true">{{item.comment_nickname}}</div>
          ...
          <div class="centen">内容:{{item.content}}</div>
        </div>
      </van-tab>
      <br />
    </van-tabs>
  ...
  mounted() {
    ...
    this.$api
      ...
      .then((res) => {
        ...
        this.arr = [this.goodsOne.image, this.goodsOne.image_path];//轮播图
        //进入详情页,则为最近浏览，存在browses库
        let browse = JSON.parse(localStorage.getItem("browses"));
        //browses为空时
        if (browse === null) {
          let i = [];
          i.push(this.goodsOne);
          localStorage.setItem("browses", JSON.stringify(i));
        } else {
          let brr = [];
          if (
            //点击项和浏览页的项没有相等的情况下
            browse.some((item) => {
              return item.id === this.goodsOne.id;
            }) === false
          ) {
            //头部添加
            browse.unshift(this.goodsOne);
            brr = browse;
            localStorage.setItem("browses", JSON.stringify(brr));
          } else {
            //点击项和浏览页的项有相等的情况下,则进行过滤
            brr = browse.filter((item1) => {
              return item1.id !== this.goodsOne.id;
            });
            brr.unshift(this.goodsOne);
            localStorage.setItem("browses", JSON.stringify(brr));
          }
        }
