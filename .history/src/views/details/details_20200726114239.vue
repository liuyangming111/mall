<template>
  <div>
    ...
    <div>
      ...
      <div class="introduce">
        ...
        <div v-if="user===null" @click="$Pop_up">
          收藏：<van-icon name="like" />
        </div>
        <div v-else>
          <div @click="Collection" v-if="arr===0">收藏：<van-icon name="like" /></div>
          <div @click="Collection" v-if="arr===1">取消收藏：<van-icon name="like" color="red"/></div>
        </div>
      </div>
    </div>
    <div class="Fabulous">
   ...
   <!-- <van-sku
      ...
      :sku="sku"
      :goods="goodsOne"
      :goods-id="goodsOne.id"
      :quota="quota"
      :hide-stock="sku.hide_stock"
      reset-stepper-on-hide
      :initial-sku="initialSku"
    >
      <template #sku-header-price="goodsOne">
        <div class="van-sku__goods-price">
          <span class="van-sku__price-symbol">￥</span>
          <span class="van-sku__price-num">{{ goodsOne.present_price }}</span>
        </div>
      </template>
      <template #sku-actions="goodsOne">
        <div class="van-sku-actions">
          <van-button
            square
            size="large"
            type="danger"
            @click="goodsOne.skuEventBus.$emit('sku:buy')"
          >买买买</van-button>
        </div>
      </template>
    </van-sku>-->
