  ...
  <div>
    ...
    <div>
      ...
      <div class="introduce">
        ...
        <div v-else>
          <div>
            <div @click="Collection" v-if='sum===0'>收藏： <van-icon name="like" /></div>
            <div @click="cancel" v-if='sum===1'>取消收藏：<van-icon name="like" class="cancel"/></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <van-goods-action>
      ...
      <van-popup v-model="show" position="bottom" closeable close-icon="close" :style="{ height: '40%'}">
...
export default {
  ...
  methods: {
    ...
    Collection() {
      this.sum=1
      this.$api.postcollection({goodsOne:this.goodsOne}).then(res=>{
        console.log(res)
      }).catch(err=>{
        console.log(err)
      })
