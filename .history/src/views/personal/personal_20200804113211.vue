<template>
  <div>
    ...
    <van-cell-group>
      ...
      <van-field v-model="date" label="出生年月" placeholder="请选择年月日" @click="showPicker = true" />
    ...
    <br/>
    <br/>
    <div class="preservations">
      <van-button type="primary" class="preservation" @click="preservation">保存</van-button>
    </div>
    <div class="preservations">
      <van-button class="preservation" @click="cancel">取消</van-button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    onConfirm(value) {
      ...
      this.userInfo.month = dayjs(value).month() + 1;
    ...
    this.$api
      ...
      .then((res) => {
        if (res.code === 200) {
          ...
          this.date = `${this.userInfo.year}年${this.userInfo.month}月${this.userInfo.day}日`;//字符拼接，年月日
        }
